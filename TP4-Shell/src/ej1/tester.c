...
void expect_write(int fd, void *buf, size_t size, const char* desc, int should_fail) {
    ...
    if (!failed && should_fail) fprintf(stderr, "      %s✘ '%s' no falló como se esperaba%s\n", RED, desc, NC);
}
...
int main() {
...

    {
        int fd[2]; pipe(fd);
        close(fd[1]);
        EXPECT_FAIL_WRITE(fd[1], &val, sizeof(val), "write en pipe con escritura cerrada");
        close(fd[0]);
    }

    {
        int fd[2]; pipe(fd);
        close(fd[0]);
        EXPECT_FAIL_WRITE(fd[1], &val, sizeof(val), "write en pipe sin lector");
        close(fd[1]);
    }
...

    {
        int fd[2]; pipe(fd);
        close(fd[1]);
        EXPECT_SUCCESS_READ(fd[0], &out, sizeof(out), "read con escritura cerrada (EOF esperado)");
        close(fd[0]);
    }
    ...
    printf("   %sCorrectos:          %d%s\n", GREEN, test_count - fail_count, NC);
    printf("   %sFallidos:           %d%s\n", RED, fail_count, NC);
    printf("%s============================================%s\n", BLUE, NC);
