...
void process_instruction() {
    // 1. Leer la instrucción desde la memoria
    uint32_t instruction = mem_read_32(CURRENT_STATE.PC);
    
    // 2. Decodificar la instrucción
    Instruction inst = decode_instruction(instruction);
    
    // 3. Mostrar la instrucción decodificada (para depuración)
    ...
    // 4. Ejecutar la instrucción según su tipo
    int branch_taken = 0;
    ...
    // 5. Avanzar el PC a la siguiente instrucción si no se tomó un salto
    if (!branch_taken) {
        NEXT_STATE.PC = CURRENT_STATE.PC + 4;
    }
    
    // 6. Actualizar el estado del CPU
