...
void execute_memory(Instruction inst) {
    switch (inst.opcode) {
        ...
        case OPCODE_LDURH: {
            ...
            uint64_t half = word & 0xFFFF;  // Extraer los primeros 16 bits
        ...
        case OPCODE_LDURB: {
            ...
            uint32_t word = mem_read_32(addr);
            uint64_t byte = word & 0xFF;  // Extraer los primeros 8 bits
...
int execute_branch(Instruction inst) {
    ...
    switch (inst.opcode) {
        ...
        case OPCODE_B_COND: {
            int cond = inst.rd; // Últimos 3 bits: condición
            int take_branch = 0;
